...
int Compute(const std::string &text) {
...
int shop::GetCashback(const std::string &login) {
    return Compute(login) % 30;
}

int shop::GetServiceFee(int price, const std::string &title, const std::string &description) {
    return Compute(std::to_string(price) + title + description) % (price / 5 + 1);
}